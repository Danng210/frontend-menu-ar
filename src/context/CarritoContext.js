    ...
    try {
      const response = await fetch('/api/realizar_pedido.php', {
        ...
        body: JSON.stringify({
          ...
          total,
          numero_mesa: numeroMesa, // Se envía el número de mesa
          nombre_cliente: nombreCliente // Se envía el nombre del cliente
        })
      });
      ...
      if (result.success) {
        ...
        // Resetear el carrito y los campos después de un pedido exitoso
        setCarrito([]);
        setCantidades({});
        setPreferencias('');
        setMetodoPago('');
